...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d("function output: ", averageLessThanMedian(List<Double>(5){i->i.toDouble() + 0.5}).toString())
        Log.d("function output: ", getView(0, View(this), List<Int>(5){i -> i + 5}, this).toString())
    }
...
//    }
    ...
    private fun averageLessThanMedian(listOfNumbers: List<Double>): Boolean =
        listOfNumbers.average() < listOfNumbers.sorted().let {
            return@let if (it.size % 2 == 0) (it[it.size/2] + it[(it.size - 1)/2]) else (it[it.size/2])
    }
